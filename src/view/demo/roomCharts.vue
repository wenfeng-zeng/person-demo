...
export default {
  ...
  methods: {
    init() {
      ...
      // let urls = process.env.NODE_ENV === 'development' ? 'wss://wfz-person.cn' : 'ws://127.0.0.1:8010'
      this.socket = io(urls, {
        path: '/room'
      })
      this.socket.on('connection', function(socket) {
      });
      this.socket.on('room chat', async function(msg) {
    ...
    async onSearch() {
      ...
      await this.socket.emit('room chat', this.keyWord);
      this.keyWord = ''
      Array.from(document.getElementsByClassName('style'))[0].scrollIntoView();
    },
  },
